...
%   (up to a limit of 275) for which nrOFDMInfo gives a default sample rate
%   of BURST.SampleRate. If the required NRB exceeds 275, then NRB = 275 is 
%   used and a larger FFT size is used to achieve a sample rate of
%   BURST.SampleRate. L is the number of OFDM symbols in half a frame.
%
...
function [waveform,grid,info] = hSSBurst(burst)
    ...
    ofdmInfo = nrOFDMInfo(burst.NRB,burst.SubcarrierSpacing,'CyclicPrefix',burst.CyclicPrefix,'Windowing',burst.Windowing,'Nfft',info.Nfft);
    symbolsPerSubframe = ofdmInfo.SymbolsPerSlot * ofdmInfo.SlotsPerSubframe;
    grid = zeros([burst.NRB*12 symbolsPerSubframe*5 1]);
    displaygrid = grid;
    ...
    % Modulate grid
    nSlot = 0;
    waveform = nrOFDMModulate(grid,burst.SubcarrierSpacing,nSlot,'CyclicPrefix',burst.CyclicPrefix,'Windowing',burst.Windowing,'Nfft',info.Nfft);
